...
rM = 
  d %>%
  #group_by(Sex) %>%
  lm(Rmax~ Sex ,family = gaussian(identity), data = .)
 
summary(rM)

```

Sex seem to be a significant factor here with a p-val of 0.00281 and smaller the slope the more significant it seem to be. 
...
```
 I did not really understand what +/-3 SD mean but looks like predictions for asymp and log asymp    looks very significant with a p-val < 2.2e-16
...
```
Autism quotient does not look linear with respect to Rmax
