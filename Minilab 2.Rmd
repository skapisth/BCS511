...
```{r selectData}
d %<>% select(Subject, Sex, Task, Rmax, Slope, C50, Asymp, LogAsymp, LogC50, VideoGaming,
              EmphatizingQuotient, SystemizingQuotient, ADHD, AutismQuotient) %>%
  na.omit(d) %>%
  distinct()
...
```{r vis}
d.summary = d
ggplot(d.summary, aes(x = Rmax, fill = as.factor(Sex))) + geom_histogram(position = "dodge")
ggplot(d.summary, aes(x = C50, fill = as.factor(Sex))) + geom_histogram(position = "dodge")
ggplot(d.summary, aes(x = Slope, fill = as.factor(Sex))) + geom_histogram(position = "dodge")
...
lm_mod1 <- lm(data = d.summary, Rmax~as.factor(Sex))
tidy(lm_mod1) #(larger Rmax, higher convergence)
lm_mod2 <- lm(data = d.summary, C50~as.factor(Sex))
tidy(lm_mod2)
lm_mod3 <- lm(data = d.summary, Slope~as.factor(Sex))
tidy(lm_mod3)

```
We conducted a linear model analysis to determine if sex plays a significant effect on the model parameters and it can be seen that
Sex seem sto be a significant factor here with a p-val < 0.05 for all parameters.
Also, larger the rmax value is higher is the convergence. Thus from the above results, we can say that females have lower Rmax than males meaning that they have higher duration threshold.
...
aSymp = 
  d %>%
  #group_by(Sex) %>%
  lm(Asymp~ Sex ,family = gaussian(identity), data = .)
...
laSymp = 
  d %>%
  lm(LogAsymp ~ Sex ,family = gaussian(identity), data = .)
 
summary(laSymp)

```
There is a significant effect of sex on Asymp but not on Log Asymp.

```{r ttest}
t.test(data = d, Asymp ~ as.factor(Sex))
t.test(data = d, LogAsymp ~ as.factor(Sex))
```

I did t-test just to see how the results look like when compared to the linear model. As it should be, we can see that sex seem to be significant Asymp but not for logAsymp.
...
```{r definingModels}
lm_aut = lm(data = d.summary, Rmax~as.factor(Sex + AutismQuotient))
lm_int = lm(data = d.summary, Rmax~as.factor(Sex + AutismQuotient)+as.factor(Sex * AutismQuotient))

```

```{r anova}
anova(lm_aut, lm_int, test = "Chisq")    

```
Interaction does not show much here.
...
```{r Ang}
d.Ang = d %>% filter(Task == "Angular")
d.Ang %>%
  ggplot(mapping = (aes(x = Rmax,fill = as.factor(Sex)))) +
  geom_histogram()
d.Ang %>%
  ggplot(mapping = (aes(x = C50,fill = as.factor(Sex)))) +
  geom_histogram()
d.Ang %>%
  ggplot(mapping = (aes(x = Slope,fill = as.factor(Sex)))) +
  geom_histogram()
```
```{r modAngRmax}
modAngRmax = d.Ang %>%
...
summary(modAngRmax)
```
We fit a linear model for the angular task to see if sex plays any significance on the model parametrs and it can be seen that it does not have a significant effect on Rmax
...
summary(modAngC50)
```
There is no significant of sex on C50 as well which can be seen from abobe where p-val == 0.6.
...
summary(modAngSlope)
```
There is no significant of sex on C50 as well which can be seen from abobe where p-val == 0.5.

```{r mask}
d.msk = d %>% filter(Task == "Mask")
d.msk%>%
  ggplot(mapping = (aes(x = Rmax,fill = as.factor(Sex)))) +
  geom_histogram()
d.msk%>%
...
Sex does not have any significant effect on either of the model parameters even in the mask task which can be seen from above where p-val > 0.05 in all the cases.


In conclusion, we can now say that for both angular and mask tasks, sex does not have any significant effect on any of the model parameters.
...
```{r fullMod}
allMod = d %>%
  lm(Rmax ~ Sex + AutismQuotient + VideoGaming + SystemizingQuotient + ADHD, data = .)
summary(allMod)

```
```{r}
mAutQ = lm(AutismQuotient ~ Sex, data = d.summary)
summary(mAutQ)
mVidGame = lm(VideoGaming ~ Sex, data = d.summary)
summary(mVidGame)
mSysQ = lm(SystemizingQuotient ~ Sex, data = d.summary)
summary(mSysQ)
mADHD = lm(ADHD ~ Sex, data = d.summary)
summary(mADHD)
```


We fit a linear model to see if sex has any significant effect on Autism Quotient, ADHD, systemizing quotient, video gaming and indiviadually sex has a significant effect on all of these parameters but when the  model is derived by summing all these factor, sex doesn't seem to have a significant effect
## Week 2
